...
def query(sql, limit):
    ...
    q = sql2Q(sql)
    if 'edition' in str(q):
        LOG('Edition was specified in the query')
        edition = None
    else:
        LOG('Edition was defaulted to 2')
        edition = 2

    if constraintsPresent(sql) or transitionsRequired(sql) or statesRequired(sql):
        species, nstates, transs, percentage = genericQuery(sql, q, limit, edition)
        nspecies = species.count()
        ntranss = transs.count()
    else:
        species = allSpeciesQuery(sql, q, limit, edition)
        nspecies = species.count()
        nstates = 0
        ntranss = 0
        transs = {}
...
def genericQuery(sql, q, limit, edition):
    """
    ...
    if edition:
        transs = models.Transitions.objects.filter(q,'finalstateindex__species__edition='+str(edition))
    else:
